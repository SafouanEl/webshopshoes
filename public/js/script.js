...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (track) {
    ...
    while (track.offsetWidth < parent.offsetWidth * 2) track.innerHTML += content;
    let pos = 0, speed = 1;
    const anim = () => {
      pos -= speed;
  ...
  openFilter?.addEventListener("click", () => { overlay.classList.add("active"); overlay.style.display = "block"; });
  ...
  function applyFilters(page = 1) {
    const brands = [...brandCbs].filter(cb => cb.checked).map(cb => cb.value.toLowerCase());
    const models = [...modelCbs].filter(cb => cb.checked).map(cb => cb.value.toLowerCase());

    const url = new URL("/api/sneakers", window.location.origin);
    if (brands.length) url.searchParams.set("brands", brands.join(","));
    if (models.length === 1) url.searchParams.set("model", models[0]);

    const gender = new URLSearchParams(window.location.search).get("gender");
    if (gender) url.searchParams.set("gender", gender.toLowerCase());
  ...
  function updatePageTitle(brands, models, count) {
    ...
    else if (brands.length === 1) pageTitle.textContent = `Alle ${brands[0]} schoenen (${count})`;
    else if (brands.length > 1) pageTitle.textContent = `Geselecteerde merken (${count})`;
    else if (gender === "heren") pageTitle.textContent = `Alle mannen schoenen (${count})`;
    else if (gender === "vrouw") pageTitle.textContent = `Alle vrouwen schoenen (${count})`;
  ...
  function updatePagination(totalPages, currentPage) {
    ...
    pag.innerHTML = Array.from({length: totalPages}, (_, i) => `
      <a href="#" class="pagination-link ${i+1 == currentPage ? "active" : ""}" data-page="${i+1}">${i+1}</a>
    `).join("");
  }
  ...
  ["desktop", "mobile"].forEach(type => {
    ...
    const suggestionsBox = document.getElementById(`${type}-search-suggestions`) || document.getElementById("search-suggestions");
    form?.addEventListener("input", async () => {
      const q = form.value.trim();
      if (!q) { suggestionsBox.classList.add("hidden"); suggestionsBox.innerHTML = ""; return; }
      const res = await fetch(`/api/search-suggest?q=${encodeURIComponent(q)}`);
      const data = await res.json();
      if (!data.length) { suggestionsBox.classList.add("hidden"); suggestionsBox.innerHTML = ""; return; }
    ...
    document.addEventListener("click", e => {
      if (suggestionsBox && !suggestionsBox.contains(e.target) && e.target !== form) {
  ...
  const updateCartCount = () => {
    ...
    if (countEl) total > 0 ? (countEl.textContent = total, countEl.classList.remove("hidden"))
  ...
  checkoutBtn?.addEventListener("click", e => { e.preventDefault(); checkoutPopup.classList.remove("hidden"); });
  popupClose?.addEventListener("click", () => checkoutPopup.classList.add("hidden"));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  slider?.addEventListener("touchend", e => {
    ...
    if (Math.abs(diff) > 50) {
      const nextIdx = diff < 0 ? (currentIdx + 1) % slides.length : (currentIdx - 1 + slides.length) % slides.length;
      showSlide(nextIdx);
    }
    startX = null;
  });
  ...
  nextBtn?.addEventListener("click", () => showSlide((currentIdx + 1) % slides.length));
  prevBtn?.addEventListener("click", () => showSlide((currentIdx - 1 + slides.length) % slides.length));
  ...
  overlay?.addEventListener("touchend", e => {
    ...
    if (Math.abs(diff) > 50) {
      showSlide(diff < 0 ? (currentIdx + 1) % slides.length : (currentIdx - 1 + slides.length) % slides.length);
    }
    overlayStartX = null;
  });
});
